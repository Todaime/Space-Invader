...
function initSprites(Choix:char):PSpriteSheet;
...
begin
    ...
    if choix= '1' then
		...
		if choix='2' then
			begin
			newSpriteSheet^.earth:=IMG_Load('ressources/FondStarWars.png');
			newSpriteSheet^.down:=IMG_Load('ressources/VaisseauStarWars.png');
			newSpriteSheet^.badguy:=IMG_Load('ressources/AlienStarWars.png');
			newSpriteSheet^.MissileAllie:=IMG_Load('ressources/MissileStarWarsAllie.png');
			newSpriteSheet^.Protection:=IMG_Load('ressources/ProtectionStarWars.png');
			newSpriteSheet^.Soucoupe:=IMG_Load('ressources/SoucoupeStarWars.png');
			newSpriteSheet^.MissileAlien:=IMG_Load('ressources/MissileStarWars.png');
			newSpriteSheet^.Alien1:=IMG_Load('ressources/AlienStarWars.png');    
			end
		else
			begin
			newSpriteSheet^.earth:=IMG_Load('ressources/FondNoel.png');
			newSpriteSheet^.down:=IMG_Load('ressources/PereNoel.png');
			newSpriteSheet^.badguy:=IMG_Load('ressources/AlienNoel.png');
			newSpriteSheet^.MissileAllie:=IMG_Load('ressources/MissileNoel.png');
			newSpriteSheet^.MissileAlien:=IMG_Load('ressources/MissileAlienNoel.png');
			newSpriteSheet^.Protection:=IMG_Load('ressources/ProtectionNoel.png');
			newSpriteSheet^.Soucoupe:=IMG_Load('ressources/SoucoupeNoel.png');
			newSpriteSheet^.Alien1:=IMG_Load('ressources/AlienNoel.png');    
			end;
...
begin
    ...
    SDL_FreeSurface(p_sprite_sheet^.Soucoupe);
    SDL_FreeSurface(p_sprite_sheet^.Alien1);
    dispose(p_sprite_sheet);
end;


{Procedure de mise en musique}
procedure son(Choix:char; var sound : pMIX_MUSIC);
begin
	...
	if Choix='1' then
		...
		if Choix='2' then 
			sound := MIX_LOADMUS('ressources/SonStarWars.mp3')
		else 
			sound := MIX_LOADMUS('ressources/SonNoel.mp3');
...
begin
       ...
       {Rectangle de la surface où viendra se placer le sprite du vaisseau}
       ...
       SDL_Flip(screen);
       ecrire(screen,'score: ' + IntToStr(ElementJeu.score), SURFACEWIDTH, SURFACEHEIGHT-30,18);
end;

procedure processKey(key:TSDL_KeyboardEvent;var ElementJeu:Element);
begin
    {Suivant la touche appuyée on le déplace ou on tire}
    {Attention, les coordonées (0,0) sont en haut à gauche}
    case key.keysym.sym of
        ...
        SDLK_RIGHT: if ElementJeu.Vaisseau.x< (SURFACEWIDTH div SPRITESIZE)-1 then
                            ElementJeu.Vaisseau.x:=ElementJeu.Vaisseau.x+1;
     {On laisse un laps de temps avant de pouvoir tirer un nouveau missile}
        SDLK_UP: if (ElementJeu.MissileAllie[ElementJeu.NbrMissileAllie].y<>ElementJeu.Vaisseau.y-1) then
							begin	
...
var k:Integer;
	begin
		k:=random(100)+1;
		{Tir aléatoire et augmentant en fonction du nombre d'alien tués}
		if (k>99+ElementJeu.NbrAlien1-8)and(ElementJeu.NbrMissileAlien<5) then
			begin
...
begin
	    ...
	    {On deplace les alien 1 de la ligne 1 et 2 or du jeu pour ne pas fausser la disjonction de cas}
	 if ElementJeu.NbrAlien1=0 then 
		ElementJeu.Alien[1][1].x:=200;
	if ElementJeu.NbrAlien2=0 
		then ElementJeu.Alien[1][2].x:=200;
	    {Disjonction des conditions pour choisir le déplacement}
	...
	randomize;
	{On appele La procédure de tir pour chaque Alien}
	for i:=1 to ElementJeu.NbrAlien1 do
		TirE(i,1,ElementJeu);
	for i:=1 to ElementJeu.NbrAlien2 do
		TirE(i,2,ElementJeu);		
...
var i,j,k:Integer;
begin
			{Supprimer missile Allie si va sortir du cadre}
	Repeat		
		for i:=1 to ElementJeu.NbrMissileAllie do
			if (ElementJeu.MissileAllie[i].y-1=-1)  then
				begin
					for j:=i to ElementJeu.NbrMissileAllie-1 do
						ElementJeu.MissileAllie[j]:=ElementJeu.MissileAllie[j+1];
					ElementJeu.NbrMissileAllie:=ElementJeu.NbrMissileAllie-1;
				end;
	until (ElementJeu.NbrMissileAllie=0) or	(ElementJeu.MissileAllie[1].y-1<>(-1));
			{Deplacer les missiles alliés de une case vers le haut}
	for i:=1 to ElementJeu.NbrMissileAllie do
		if ((ElementJeu.MissileAllie[i].y-1)<>(-1)) then
			ElementJeu.MissileAllie[i].y:=ElementJeu.MissileAllie[i].y-1;
		{Test Colision Missile }
	for i:=1 to ElementJeu.NbrMissileAllie do
		for j:=1 to ElementJeu.NbrMissileAlien do
			if ((ElementJeu.MissileAllie[i].y=ElementJeu.MissileAlien[j].y)or(ElementJeu.MissileAllie[i].y-1=ElementJeu.MissileAlien[j].y)) and (ElementJeu.MissileAllie[i].x = ElementJeu.MissileAlien[j].x) then
				begin
...
Procedure MvtSoucoupe(var ElementJeu:Element);
	begin
	{ Apparait a un interval de tour regulier}
...
var x,y,i:Integer;
	begin
		...
		{Placement initial des deux lignes aliens}
		for y:=0 to 1 do
			for i:=1 to 8 do
				begin
 ...
 procedure jeu(Choix:Char; fenetre:PSDL_SURFACE; var ElementJeu:Element);
 var
		...
		repeat
			...
			for i:=1 to ElementJeu.NbrMissileAlien do
				if (ElementJeu.MissileAlien[i].y=ElementJeu.vaisseau.y)and(ElementJeu.Vaisseau.x=ElementJeu.MissileAlien[i].x) then
					Fin:=True;
			drawScene(fenetre,sprites,ElementJeu);
		Until (ElementJeu.Alien[1][1].y=ElementJeu.Vaisseau.y) or  (ElementJeu.Alien[1][2].y=ElementJeu.Vaisseau.y) or((ElementJeu.NbrAlien1=0)and(ElementJeu.NbrAlien2=0)) or (Fin=True);
		q:=q+40;
	Until (ElementJeu.Alien[1][1].y=ElementJeu.Vaisseau.y)or(ElementJeu.Alien[1][2].y=ElementJeu.vaisseau.y) or (Fin=True);
...
Procedure GestionScore(ElementJeu:Element);
	...
	begin
		Pseudo:='Unknow';
		{Si le fichier n'existe pas, créer le fichier et ajoute le score}
...
Procedure Acceuil(var Reponse:Integer);
	begin
			clrscr;
			TextColor(Brown);
			GotoXY(18,1);
...
begin
	...
	Writeln('!!!Des envahisseurs ont été détectés à la frontière de notre galaxie!!! ');
	Writeln(' A bord de votre vaisseau X-9789 votre mission est de venir au secour de l''humanite');
	Writeln('Déplacez vous  et tirez a l"aide des touches directionnelles ');
	writeln('');
	writeln('(Press a key to return to the menu)');
	readln(key);
end;

procedure ChoixTheme(var Choix:char);
begin
	repeat
			Clrscr;
			writeln( 'Theme 1 : Classic (1)');
			writeln('Theme 2 : Star Wars (2)');
			writeln('Theme 3 : Christmas (3)');
			read (Choix);
